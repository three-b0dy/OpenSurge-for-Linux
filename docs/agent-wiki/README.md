...
arm64, IPv4 gateway behavior, mihomo TUN, iproute2, nftables, sysctl, dnsmasq
where configured, and installed systemd gateway/control services. Release
`.deb` packages provide the supported installation boundary.
...
path; use `make linux-lab-test` and `make linux-lab-test-tun` on a Linux host
with root network-namespace support for those respective evidence claims.
